 ...
 * 四叉树结构<p>
 * 
 * 该组件存放Renderable用于渲染 调用insert方法添加对象
 * 调用build方法构建四叉树<p>
 * 
 * 构建之后四叉树结构不会变化，添加新的物体只会更新节点内容
 * 调用query用于查找范围内对象<p>
 * 
 * 
 * 使用的例子:
 ...
 * qt.setMaxDepth(5);		//设定四叉树最大深度为5
 * qt.setMaxObject(8);		//设定理想对象数量为8
 * 
 * for(QuadTreeObject r : objList) {		//添加对象
 * 	qt.insert(r);
 * }
 * 
 * qt.build();		//创建四叉树
 * 
 * qt.query(rect, queryListener);	//查询
 * 
 * </pre>
 * @author xu jun
...
//							System.out.println("将对象obj" + n + obj + " 添加到[线段]节点 [" + i + "]");
					...
					for(int i = 0; i < children.length; ++i) {
						if(children[i].insert(obj, currDepth)) {
							//添加成功
//							System.out.println("将对象obj" + n + obj +" 添加到[空间]节点 [" + i + "]");
							objs.removeIndex(n);
							break;
						}
					}
...
//		System.out.println("线性查询结束 结果 = " + ret.size + " 时间 = " + (System.nanoTime() - time));
		...
		System.out.println("四叉树查询结束 结果 = " + ret.size + 
